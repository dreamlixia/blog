...
---
1. **路由原理**  
   - **Hash 路由**：通过 URL 中的 `#`（即 hash 值）来模拟不同的页面状态。当 hash 值改变时，浏览器不会向服务器发送请求，而是触发 `hashchange` 事件，前端根据这个变化加载对应的组件。  
   - **History 路由**：依赖 HTML5 History API（如 `pushState` 和 `popState`）来改变 URL，并不会刷新页面。这样可以在不重新加载页面的前提下更新 URL，使得地址栏中显示真实的路径。

2. **URL 表现形式**  
   - **Hash 路由**：URL 中会包含 `#` 符号，例如 `http://example.com/#/home`。  
   - **History 路由**：URL 更加友好和干净，没有 `#` 符号，例如 `http://example.com/home`。

3. **浏览器兼容性**  
   - **Hash 路由**：兼容性较好，几乎支持所有浏览器，包括一些老旧的版本。  
   - **History 路由**：需要浏览器支持 HTML5 History API（IE10 以上的版本均支持），在非常老的浏览器中可能会有兼容性问题。

4. **服务端配置要求**  
   - **Hash 路由**：由于 hash 部分不会被发送到服务器，前后端分离时无需做额外的服务端配置。  
   - **History 路由**：由于用户直接访问某个子路径时（如 `http://example.com/home`），服务器会收到该请求并尝试匹配对应的资源。因此，需要在服务器端配置路由重定向，将所有请求都指向应用的入口文件（通常是 `index.html`），以便前端路由能够接管处理。

5. **SEO 与用户体验**  
   - **Hash 路由**：对 SEO 的优化能力较弱，因为爬虫通常不会索引 `#` 后面的内容。此外，URL 结构较为臃肿。  
   - **History 路由**：生成的 URL 更加清晰，对 SEO 更友好，有助于搜索引擎更好地抓取和索引页面内容，但同时也要求服务端能正确处理请求。

总体来说，hash 路由简单易用，兼容性好，适合一些不需要服务端额外支持的小型项目；而 history 路由则更符合现代 web 开发对 URL 美观性和 SEO 的要求，但需要额外配置服务器以支持单页面应用的刷新和深链接。
...
12. forEach()： 没有返回值, 改变原数组；
    > forEach 方法提供了一种简单的数组迭代方法，但它缺乏中断或停止中循环的灵活性。不可以使用 break 或者 return，会报错，可以使用 for of、 some() 和 every() 方法代替，可以使用 break 退出循环。
...
- replacer（可选）：
    ...
    ```
    - 如果该参数是 null，或者未提供，则对象所有的属性都会被序列化。
