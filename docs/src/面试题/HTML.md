...
---
四个单位值的大小都是动态的。
- vw：viewport width 视口宽度 具体的单位大小都是各自的百分之一，所以 1vw = 1/100, 1vh = 1/100
- vh：viewport height 视口高度
- vw和vh一起使用，因为在推动窗口的时候希望按比例缩放的
- em：父元素字体的大小，是一个相对单位。默认为浏览器默认字体大小值16px， 1em=16px， 2em=32px；
    如果设置了body元素的字体大小为20px，则1em=20px；2em=40px；
- 父元素设置完em的大小会影响自己子元素的em的值。
- rem：仍然是相对大小，但相对的是html元素的字体大小。基于html，不会被中间的元素所影响。定义html的font-size：24px，其子孙元素都不会影响，em会影响。通过它既可以做到只修改根元素就成比例地调整所有字体大小，又可以避免字体大小逐层复合的连锁反应。
