<!--
 * @Author: dreamlixia 1763112943@qq.com
 * @Date: 2022-06-21 17:46:45
 * @LastEditors: dreamlixia 1763112943@qq.com
 * @LastEditTime: 2022-07-01 00:17:02
 * @FilePath: /MyBlog/blog/docs/README.md
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
-->

[//]: ![](https://github.com/dreamlixia/blog/raw/master/images/title.jpeg) 
[//]:src="./images/title.jpeg"

<div style="text-align:center">
    <img width="50%" src="https://github.com/dreamlixia/blog/raw/master/images/title.jpeg" alt="一张猫猫图片" />
    <div style="font-size: 20px; margin-top: 30px!important; margin-bottom: 30px">欢迎来到我的博客空间</div>
    <div style="width: 100%; height: 1px; border-top: 1px solid #dcdcdc; margin-bottom: 30px"></div>
</div> 

<div style="display: -webkit-flex; display: flex; flex-direction: row; flex-wrap: wrap; justify-content: flex-start; align-items: center; align-content: center; font-size: 16px; width: 100%">
    <span style="box-sizing: border-box; width: 50%; border-right: 1px solid rgb(220, 220, 220)">付出的越多，得到的就会更多。</span>
    <span style="box-sizing: border-box; width: 49%; padding-left: 10px;">Don't peek at my blog……</span>
</div>
<div style="width: 100%; margin: 10px 0;">
    <a href="#/src/%E5%89%8D%E7%AB%AF/%E5%9F%BA%E7%A1%80%E7%9F%A5%E8%AF%86/DNS.html" style="font-weight: 400">开始浏览博客内容，biu～</a>
</div>

<div style="font-size: 12px;color: rgb(153, 153, 153);text-align: center;position: fixed;left: 0;bottom: 0;width: 100%;margin: 10px 0">lynsey's blog Copyright © 2022 Website author. All Rights Reserved.</div>


<!-- ---
title: 目录导航
author: jinglixia
tags:
- web
- PC
- 后台
--- -->